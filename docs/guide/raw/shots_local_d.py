...
with sync_playwright() as p:
    log("=== LOCAL session D (hourly) start ===")
    ...
    # 13. hourly sidebar
    collapse_all(pg); sidebar_top(pg)
    shot(pg, "34_local_hourly_sidebar_engine_data.png", full=False)
    for i, lab in enumerate(["Position sizing", "Hedge policy", "Cost model"]):
        collapse_all(pg); set_expander(pg, lab, True)
        e = expander(pg, lab); e.scroll_into_view_if_needed(); time.sleep(0.8)
        log(f"hourly expander {lab!r}: {e.inner_text().replace(chr(10),' | ')}")
        shot(pg, f"{35+i:02d}_local_hourly_sidebar_{lab.lower().replace(' ','_')}.png", full=False)
    collapse_all(pg)
    ...
    log(f"slider text: {sl.inner_text().replace(chr(10),' | ')}; aria {sl.locator('[role=slider]').first.get_attribute('aria-valuenow')}")
    ...
    sl = pg.locator('[data-testid="stSlider"]', has_text="Total price move").first
    th = sl.locator('[role="slider"]').first; th.scroll_into_view_if_needed(); th.focus()
    for _ in range(10):
        th.press("ArrowLeft"); time.sleep(0.15)
    settle(pg, 2)
    v = sl.locator('[role="slider"]').first.get_attribute('aria-valuenow'); log(f"slider now: {v}")
    if v != "-30":
        # fallback: keyboard again
        cur = int(float(v)); th = sl.locator('[role="slider"]').first; th.focus()
        for _ in range(abs(cur + 30)): th.press("ArrowLeft" if cur > -30 else "ArrowRight"); time.sleep(0.15)
        settle(pg, 2); v = sl.locator('[role="slider"]').first.get_attribute('aria-valuenow'); log(f"slider now (2nd try): {v}")

...
with sync_playwright() as p:
    ...
    import shutil; shutil.copy(src, f"{SHOTS}/19_local_run_default_full.png"); log(f"screenshot 19_local_run_default_full.png ({os.path.getsize(src)} bytes) viewport 1600x{h}")
    # measure
    h3s = pg.locator('[data-testid="stMain"] h3')
    names = h3s.all_inner_texts(); log(f"main h3s: {names}")
    hb = main_h3(pg, "APR")
    ...
    mb = [ms.nth(i).bounding_box() for i in range(ms.count())]
    y_cum = ybox(main_h3(pg, "Cumulative PnL")); y_attr = ybox(main_h3(pg, "PnL attribution")); y_eth = ybox(main_h3(pg, "ETH price and LP delta"))
    y_sum = ybox(expander(pg, "Full summary"))
    crop(src, "20_local_run_verdict_banner.png", ybox(hb)[0], ybox(cap)[1], pg)
    crop(src, "21_local_run_metric_tiles.png", min(x["y"] for x in mb), max(x["y"]+x["height"] for x in mb), pg)
    crop(src, "22_local_run_cumulative_pnl.png", y_cum[0], y_attr[0] - 24, pg)
    crop(src, "23_local_run_attribution_cumulative.png", y_attr[0], y_eth[0] - 24, pg)
    crop(src, "25_local_run_eth_price_lp_delta.png", y_eth[0], y_sum[0] - 16, pg)
    click_tab(pg, "Daily bars"); time.sleep(3)
    src2 = page_png(pg, "run7_daily.png")
    y_attr = ybox(main_h3(pg, "PnL attribution")); y_eth = ybox(main_h3(pg, "ETH price and LP delta"))
    crop(src2, "24_local_run_attribution_daily_bars.png", y_attr[0], y_eth[0] - 24, pg)
    click_tab(pg, "Cumulative"); time.sleep(2)
    # expanders

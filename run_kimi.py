...
class UniswapV3Math:
    ...
    @classmethod
    def get_amounts(
        cls, tick: int, tick_a: int, tick_b: int, liquidity: float
    ) -> Tuple[float, float]:
        """
    ...
    @classmethod
    def get_liquidity(
        cls, tick: int, tick_a: int, tick_b: int, amount0: float, amount1: float
    ) -> float:
    ...
    @classmethod
    def get_value_usd(
        cls, tick: int, tick_a: int, tick_b: int, liquidity: float, price: float
    ) -> float:
...
class DeltaHedgeBacktest:
    ...
    def run(
        ...
        pool_volume_data: Optional[pd.DataFrame] = None,
    ) -> pd.DataFrame:
        """
        Run the backtest.
        ...
        for i, (ts, row) in enumerate(df.iterrows()):
            ...
            if abs(hedge_eth) > 0:
                ...
                funding_pnl = (
                    notional * funding_rate_period * (-1 if hedge_eth < 0 else 1)
                )
            ...
            if should_rebalance:
                ...
                if abs(trade_size) > 1e-12:
                    ...
                    if abs(hedge_eth) > 0 and np.sign(trade_size) != np.sign(hedge_eth):
                        ...
                        trade_pnl = (
                            -np.sign(hedge_eth) * close_size * (price - hedge_avg_price)
                        )
                    ...
                    if abs(new_hedge) < 1e-12:
                        ...
                        if (
                            np.sign(new_hedge) == np.sign(hedge_eth)
                            and abs(hedge_eth) > 0
                        ):
                            hedge_avg_price = (
                                hedge_eth * hedge_avg_price + trade_size * price
                            ) / new_hedge
                        else:
            ...
            results.append(
                {
                    ...
                    "binance_fees": (
                        cum_binance_fees if should_rebalance and i > 0 else 0.0
                    ),
                    "cum_binance_fees": cum_binance_fees,
                    "slippage_cost": (
                        cum_slippage if should_rebalance and i > 0 else 0.0
                    ),
    ...
    def summarize(self, results: pd.DataFrame) -> dict:
        ...
        summary = {
            ...
            "net_apr": (
                (final["total_pnl"] / self.cfg.initial_capital_usd / years) * 100
                if years > 0
                else 0
            ),
        ...
        hedge_costs = (
            abs(final["cum_funding"])
            + final["cum_binance_fees"]
            + final["cum_slippage"]
        )
...
def main():
    ...
    print(
        f"  Net P&L:                ${summary['net_pnl']:>12,.2f} ({summary['net_pnl_pct']:.2f}%)"
    )
